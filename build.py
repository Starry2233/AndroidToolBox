...
def write_build_info_module(metadata: dict[str, str], target_path: str) -> None:
    allow_xtc = str(metadata.get("persist.atb.xtc.allow", "False")).lower() in ("1", "true", "yes", "y")
    lines = [
        "# -*- coding: utf-8 -*-",
        '"""Build-time locked metadata. Generated by build.py."""',
        "",
        f'BUILD_TYPE = {metadata.get("ro.build.type", "release")!r}',
        f'SOFT_VERSION = {metadata.get("ro.product.current.softversion", "")!r}',
        f'BUILD_VERSION = {metadata.get("ro.build.version", "")!r}',
        f'PRODUCT_COMMIT = {metadata.get("ro.product.commit", "")!r}',
        f'BUILD_DATE_UTC = {metadata.get("ro.build.date.utc", "0")!r}',
